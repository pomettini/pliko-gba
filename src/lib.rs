...
impl Pliko {
    ...
    fn set_small(&mut self, small: &Tag) {
    ...
    fn do_action(&mut self, action: ActionType, player: &mut Player) {
        player.perform_action(action);
        self.reset();
    }

    fn draw(&mut self, frame: &mut GraphicsFrame<'_>) {
